  ...
  let plugins = [routes.browserRouteModulesPlugin(ctx, /\?browser$/), deprecatedRemixPackage.deprecatedRemixPackagePlugin(ctx), cssBundlePlugin.cssBundlePlugin(refs), cssModuleImports.cssModulesPlugin(ctx, {
    ...
    setup: build => {
      build.onResolve({
        ...
        if (ctx.options.onWarning && !isNodeBuiltIn(packageName) && !/\bnode_modules\b/.test(args.importer) && !args.path.endsWith(".css") && !cssSideEffectImports.isCssSideEffectImportPath(args.path) && (
